...
poetry run raft-cli generate "$DATA_DIRECTORY/qd61g27j" "$OUTPUT_DIRECTORY" "$CONFIG_FILE" \
  --prefix "qd61g27j"
poetry run raft-cli generate "$DATA_DIRECTORY/qdc5ghs3" "$OUTPUT_DIRECTORY" "$CONFIG_FILE" \
  --prefix "qdc5ghs3"
poetry run raft-cli generate "$DATA_DIRECTORY/qdch0ftq" "$OUTPUT_DIRECTORY" "$CONFIG_FILE" \
  --prefix "qdch0ftq"
poetry run raft-cli generate "$DATA_DIRECTORY/qdchdmy1" "$OUTPUT_DIRECTORY" "$CONFIG_FILE" \
  --prefix "qdchdmy1"
