...
def create_query_item(name: str, directory: Path, query_data: FileQueryData) -> QueryItem:
...
def make_selection_paths_relative(selection: SelectionItem, reference: Path) -> SelectionItem:
...
def generate_metafiles(context: MetafileGenerationContext, config: Path) -> None:
    ...
    for name, group_config in config["groups"].items():
        ...
        for target_name, target_directory in targets.items():
            queries.append(create_query_item(target_name, target_directory, file_query_data))
        
        query_results: list[Result[SelectionItem, str]] = [execute_query(query) for query in queries]

        # Acquire the selections from successful queries
        selections: list[SelectionItem] = [result.ok() for result in query_results if result.is_ok()]

        # Make file paths in selection relative to the root directory
        selections: list[SelectionItem] = [
            make_selection_paths_relative(selection, context.root_directory) for selection in selections
        ]

        # Serialize selection data to write to file
        serialized_groups[name] = [serialize_selection(selection) for selection in selections]
